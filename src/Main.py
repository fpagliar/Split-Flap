...
def parseOption(config, status, option):
  if option == "--position":
      ...
      calibrator = Calibrator(Pin.GetPin, config)
      calibrator.calibrateInitialPosition()
      calibrator.calibrateTicksPerLetter(status, SystemCalibration(config.numberOfMotors()))
  elif option == "--recalibrate":
      calibrator = Calibrator(Pin.GetPin, config)
      calibrator.calibrateInitialPosition()
      systemCalibration = SystemCalibration(config.numberOfMotors())
      systemCalibration.cleanup()
      calibrator.calibrateTicksPerLetter(status, systemCalibration)
  elif option == "--message":
      display = DisplayFactory(Pin.GetPin, config).build(status)
      display.show(sys.argv[2])
  elif option == "--show":
      display = DisplayFactory(Pin.GetPin, config).buildDisplay(status, SystemCalibration(config.numberOfMotors()))
      while True:
...
if __name__ == "__main__":
  ...
  status = SystemStatus(config.numberOfMotors())
  try:
      parseOption(config, status, option)
  except:
      status.save()
